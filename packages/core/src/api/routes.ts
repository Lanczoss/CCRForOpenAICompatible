...
import { isContextOverflowError } from "@/utils/errorParser";
...
async function handleTransformerEndpoint(
  ...
  try {
    ...
    const { requestBody, config, bypass } = await processRequestTransformers(
      ...
      {
        req,
        isRetry: isRetryAttempt // 传递重试标志给 Transformer
      }
    );
    ...
    // --- 工业级应急重试机制 ---
    // 如果是 Token 溢出且尚未重试过
    if (!isRetryAttempt && isContextOverflowError(error.message || "")) {
      fastify.log.warn(`[context_overflow] Detected overflow for session ${sessionId}. Triggering auto-retry with aggressive pruning.`);
      // 清理当前追踪状态
      RequestTracker.release(sessionId);
      // 立即发起第二次尝试 (带上 isRetryAttempt = true)
      return handleTransformerEndpoint(req, reply, fastify, transformer, true);
    }
...
async function sendRequestToProvider(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (isRetryable && !isRetryAttempt) {
        const retryAfter = response.headers.get("Retry-After");
        let delayMs = 2000; // 默认 2s 步进
        ...
        fastify.log.warn(`[provider_retry] Provider ${provider.name} returned ${response.status}. Retrying after ${delayMs}ms...`);
        
        await new Promise(r => setTimeout(r, delayMs));
        
        // 递归执行一次重试 (由 handleTransformerEndpoint 的 isRetryAttempt 标志控制总数)
        // 注意：此处我们直接返回错误，由外层 handleTransformerEndpoint 的 catch 块统一驱动重试流程更稳健
      }
